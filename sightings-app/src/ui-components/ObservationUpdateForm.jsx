...
import { getObservation } from "../graphql/queries";
...
export default function ObservationUpdateForm(props) {
  const {
    id: idProp,
    observation: observationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [urlLinks, setUrlLinks] = React.useState(initialValues.urlLinks);
  const [byUser, setByUser] = React.useState(initialValues.byUser);
  const [group, setGroup] = React.useState(initialValues.group);
  const [checkedByUser, setCheckedByUser] = React.useState(
    initialValues.checkedByUser
  );
  ...
  const resetStateValues = () => {
    ...
    setTimeName(cleanValues.timeName);
    setReportType(cleanValues.reportType);
    setSite(cleanValues.site);
    setCountry(cleanValues.country);
    setSpecieCommonName(cleanValues.specieCommonName);
    setSpecie(cleanValues.specie);
    ...
    setUrlLinks(cleanValues.urlLinks ?? []);
    setCurrentUrlLinksValue("");
    setByUser(cleanValues.byUser);
    setGroup(cleanValues.group);
    setCheckedByUser(cleanValues.checkedByUser);
  ...
  const [observationRecord, setObservationRecord] =
    React.useState(observationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getObservation.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getObservation
        : observationModelProp;
      setObservationRecord(record);
    };
    queryData();
  }, [idProp, observationModelProp]);
  React.useEffect(resetStateValues, [observationRecord]);
  const [currentUrlLinksValue, setCurrentUrlLinksValue] = React.useState("");
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          date,
          time: time ?? null,
          timeName: timeName ?? null,
          reportType: reportType ?? null,
          site: site ?? null,
          country: country ?? null,
          specieCommonName: specieCommonName ?? null,
          specie: specie ?? null,
          count,
          ...
          byUser: byUser ?? null,
          group: group ?? null,
          checkedByUser: checkedByUser ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateObservation.replaceAll("__typename", ""),
            variables: {
              input: {
    ...
    >
      ...
      <ArrayField
        onChange={async (items) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            values = result?.urlLinks ?? values;
          }
          setUrlLinks(values);
          setCurrentUrlLinksValue("");
        }}
        currentFieldValue={currentUrlLinksValue}
        label={"Url links"}
        items={urlLinks}
        hasError={errors?.urlLinks?.hasError}
        runValidationTasks={async () =>
          await runValidationTasks("urlLinks", currentUrlLinksValue)
        }
        errorMessage={errors?.urlLinks?.errorMessage}
        setFieldValue={setCurrentUrlLinksValue}
        inputFieldRef={urlLinksRef}
        defaultFieldValue={""}
      >
        <TextField
          label="Url links"
          isRequired={false}
          isReadOnly={false}
          value={currentUrlLinksValue}
          onChange={(e) => {
            let { value } = e.target;
            if (errors.urlLinks?.hasError) {
              runValidationTasks("urlLinks", value);
            }
            setCurrentUrlLinksValue(value);
          }}
          onBlur={() => runValidationTasks("urlLinks", currentUrlLinksValue)}
          errorMessage={errors.urlLinks?.errorMessage}
          hasError={errors.urlLinks?.hasError}
          ref={urlLinksRef}
          labelHidden={true}
          {...getOverrideProps(overrides, "urlLinks")}
        ></TextField>
      </ArrayField>
      <TextField
        label="By user"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.checkedByUser ?? value;
          }
          if (errors.checkedByUser?.hasError) {
            runValidationTasks("checkedByUser", value);
          }
          setCheckedByUser(value);
        }}
        onBlur={() => runValidationTasks("checkedByUser", checkedByUser)}
        errorMessage={errors.checkedByUser?.errorMessage}
        hasError={errors.checkedByUser?.hasError}
        {...getOverrideProps(overrides, "checkedByUser")}
      ></TextField>
      <ArrayField
        onChange={async (items) => {
          let values = items;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || observationModelProp)}
